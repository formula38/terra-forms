##############################################
# main-soc2.tf - AWS Infrastructure for SOC 2 Compliance
##############################################
terraform {
  required_providers {
    aws = {
...
# S3 Buckets for SOC 2 data and logs
resource "aws_s3_bucket" "soc2_data_bucket" {
  bucket = "soc2-data-bucket-${random_string.bucket_suffix.result}"
  tags   = { Name = "soc2-data-bucket", Environment = var.environment }
}

resource "aws_s3_bucket_acl" "soc2_data_bucket" {
...
resource "aws_s3_bucket" "soc2_log_bucket" {
  ...
  tags   = { Name = "soc2-log-bucket", Environment = var.environment }
}

resource "aws_s3_bucket_acl" "soc2_log_bucket" {
...
# IAM Role for SOC 2 EC2 with strict least privilege
...
# EC2 Instance for SOC 2 with encrypted storage
resource "aws_instance" "soc2_ec2" {
  ...
  EOF
  tags = { Name = "soc2-ec2", Environment = var.environment }
}

resource "aws_security_group" "soc2_sg" {
  name        = "soc2_sg"
  description = "Security group for SOC 2 compliant resources"
  vpc_id      = aws_vpc.soc2_vpc.id
