##############################################
# main-pci_dss.tf - AWS Infrastructure for PCI DSS Compliance
##############################################
terraform {
  required_providers {
    aws = {
  ...
  backend "s3" {
    ...
    key            = "terraform/state/pci_dss.tfstate"
    region         = "us-west-1"
    encrypt        = true
    use_lockfile   = true
  }
...
# VPC Flow Logs for PCI environment
...
# S3 Buckets for PCI data and logs
...
resource "aws_s3_bucket_acl" "pci_data_bucket" {
...
resource "aws_s3_bucket_acl" "pci_log_bucket" {
...
# IAM Role for PCI EC2 with strict least privilege
...
# EC2 Instance in the CDE with encrypted storage
