...
terraform {
  ...
  backend "s3" {
    bucket         = "tf-state-sox"            # Replace with your remote state bucket name
    key            = "terraform/state/sox.tfstate"
    region         = var.region
    encrypt        = true
    dynamodb_table = "tf-locks-sox"            # Replace with your DynamoDB table for state locking
  }
}
...
variable "trusted_ip_range" {
  description = "Trusted IP range for administrative access"
...
resource "random_string" "bucket_suffix" {
  description = "Random suffix for unique S3 bucket names"
  length      = 8
  special     = false
  upper       = false
}
...
resource "random_string" "log_bucket_suffix" {
  description = "Random suffix for unique S3 log bucket names"
  length      = 8
  special     = false
  upper       = false
}
...
##############################################
# Encryption: KMS Key with Strict Policies
##############################################
...
##############################################
# IAM: Roles, Policies & Instance Profiles (Least Privilege)
##############################################
...
##############################################
# Compute: EC2 Instance with Encrypted EBS
##############################################
resource "aws_instance" "sox_ec2" {
  ami                    = "ami-0c55b159cbfafe1f0"   # Replace with your approved AMI
...
##############################################
# Security Group: Strict Ingress/Egress Controls
##############################################
...
##############################################
# Database: RDS PostgreSQL Instance with Encryption & High Availability
##############################################
...
##############################################
# Monitoring & Audit: AWS Config for Continuous Resource Compliance
##############################################
