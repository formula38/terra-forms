...
resource "aws_subnet" "fedramp_subnet_b" {
  ...
  tags = { Name = "fedramp-subnet-b" }
}

resource "aws_internet_gateway" "fedramp_igw" {
  vpc_id = aws_vpc.fedramp_vpc.id
  tags   = { Name = "fedramp-igw" }
}
...
# KMS Key for encryption with FedRAMP controls
...
resource "aws_s3_bucket" "fedramp_data_bucket" {
  ...
  tags   = { Name = "fedramp-data-bucket", Environment = var.environment }
}

resource "aws_s3_bucket_acl" "fedramp_data_bucket" {
...
resource "aws_s3_bucket" "fedramp_log_bucket" {
  ...
  tags   = { Name = "fedramp-log-bucket", Environment = var.environment }
}

resource "aws_s3_bucket_acl" "fedramp_log_bucket" {
...
resource "aws_instance" "fedramp_ec2" {
  ...
  EOF
  tags = { Name = "fedramp-ec2", Environment = var.environment }
}
...
resource "aws_security_group" "fedramp_sg" {
  ...
  tags = { Name = "fedramp_sg", Environment = var.environment }
}
...
resource "aws_db_subnet_group" "fedramp_db_subnet" {
  ...
  tags       = { Name = "fedramp-db-subnet", Environment = var.environment }
}
